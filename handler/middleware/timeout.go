...
func Timeout(timeout time.Duration, errTimeout *errors.MathSheetsError) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		case <-panicChan:
			...
			mathSheetErr := errors.NewInternalServerError("did not recover from panic in side timeout")
			tw.ResponseWriter.WriteHeader(mathSheetErr.Status)
			eResp, _ := json.Marshal(mathSheetErr)
